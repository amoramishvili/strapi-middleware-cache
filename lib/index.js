...
const Cache = (strapi) => {
  ...
  const withStrapiMiddleware = _.get(
    options,
    "populateStrapiMiddleware",
    false
  );
  ...
  return {
    ...
    initialize() {
      ...
      const clearCache = async ({ model, id = null }) => {
        ...
        await Promise.all(_.filter(keys, shouldBust).map(bust));
      };
      ...
      if (withStrapiMiddleware) {
        _.set(strapi, "middleware.memcache", {
          bust: clearCache,
          store: cache,
        });
      }
      ...
      router.post(
        "/content-manager/collection-types/:scope/publish/:id*",
        bustAdmin
      );
      router.post(
        "/content-manager/collection-types/:scope/unpublish/:id*",
        bustAdmin
      );
      router.post(
        `/content-manager/${type}/:scope/:id*/actions/publish`,
        bustAdmin
      );
      router.post(
        `/content-manager/${type}/:scope/:id*/actions/unpublish`,
        bustAdmin
      );
